Verhaeghe_deg_df <- read.table("data/differential_expression_data/Verhaeghe_DEG_limma_unique_HGNC.txt",
                               sep = "\t", 
                               header = T)

Voisin_deg_df <- read.table("data/differential_expression_data/Voisin_DEG_limma_unique_HGNC.txt",
                            sep = "\t", 
                            header = T)

Ogilvie_nasal_deg_df <- read.table("data/differential_expression_data/Ogilvie_nasal_DEG_limma_unique_HGNC.csv",
                                   sep = "\t", 
                                   header = T)

Clarke_deg_df <- read.table("data/differential_expression_data/Clarke_DEG_limma_unique_HGNC.csv",
                            sep = "\t", 
                            header = T)

Ogilvie_bronchial_deg_df <- read.table("data/differential_expression_data/Ogilvie_bronchial_DEG_limma_unique_HGNC.csv",
                                       sep = "\t", 
                                       header = T)
...
Balloy_deg_df <- read.table("data/differential_expression_data/Balloy_without_CTRL2_DEG_limma_unique_HGNC.txt",
                            sep = "\t", 
                            header = T)

Zoso_deg_df <- read.table("data/differential_expression_data/Zoso_DEG_limma_unique_HGNC.txt", 
                          sep = "\t", 
                          header = T)
...
Ling_deg_df <- read.table("data/differential_expression_data/Ling_normalised_differential_expression_analysis_filtered_limma.txt",
                          sep = "\t", 
                          header = T)
...
Saint_Criq_SC_deg_df <- read.table("data/differential_expression_data/Saint_Criq_SC_DEG_limma_unique_HGNC.txt",
                                   sep = "\t", 
                                   header = T)
Saint_Criq_UNC_deg_df <- read.table("data/differential_expression_data/Saint_Criq_UNC_without_NCF_2_DEG_limma_unique_HGNC.txt",
                                    sep = "\t", 
                                    header = T)
